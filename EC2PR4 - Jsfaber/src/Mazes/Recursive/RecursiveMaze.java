...
public class RecursiveMaze implements Maze {
    ...
    public RecursiveMaze(int size, int startX, int startY){
        width = size;
        height = size;
        maze = new RecursiveCell[width][height];
        rnd = new Random();
        for(int i = 0; i < width; i++){
            for(int j = 0; j < height; j++){
                maze[i][j] = new RecursiveCell();
            }
        }
    ...
    private void digTile(int x, int y){
        RecursiveCell currentCell = maze[x][y];
        ...
        while(neighboursAvailable) {
            ...
            switch (nextDirection) {
                case NORTH:
                    RecursiveCell northCell = maze[x][y-1];
                    ...
                    break;
                case SOUTH:
                    RecursiveCell southCell = maze[x][y+1];
                    availableDirections.remove(nextDirection);
                    ...
                    break;
                case WEST:
                    RecursiveCell westCell = maze[x-1][y];
                    availableDirections.remove(nextDirection);
                    ...
                    break;
                case EAST:
                    RecursiveCell eastCell = maze[x+1][y];
                    availableDirections.remove(nextDirection);
